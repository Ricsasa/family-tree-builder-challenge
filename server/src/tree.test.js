...
const {
  addPerson, addParentEdge, addSpouseEdge, findPerson, readGraph,
  renamePerson, replaceParent, removePerson,
...
test("addPerson refuses a duplicate name and returns the existing people", () => {
  const id = idOf("Juan");

  const refusal = addPerson({ name: "Juan" });
  assert.equal(refusal.ok, false);
  assert.deepEqual(refusal.existing.map((p) => p.id), [id]);
